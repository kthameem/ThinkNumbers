package com.thinknumbers.testcases;

import org.testng.Assert;
import org.testng.annotations.AfterMethod;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.DataProvider;
import org.testng.annotations.Test;
import org.testng.asserts.SoftAssert;

import com.thinknumbers.base.BaseTest;
import com.thinknumbers.pages.HomePage;
import com.thinknumbers.pages.LoginPage;
import com.thinknumbers.pages.MasterPage;
import com.thinknumbers.pages.RLReportingLedgerListsPage;
import com.thinknumbers.pages.ThinkiamHomePage;
import com.thinknumbers.util.TestUtil;

public class RLReportingLedgerListsPageTest extends BaseTest{
	
	LoginPage loginPage;
	ThinkiamHomePage thinkiamHomePage;
	HomePage homePage;
	MasterPage masterPage;
	RLReportingLedgerListsPage rLReportingLedgerListsPage;
	
	String sheetName = "ReportingLedger";
	
	public RLReportingLedgerListsPageTest() {
		super();
	}
	
	@BeforeMethod
	public void setUp() throws InterruptedException {
		initialization();
		softAssert = new SoftAssert();
		loginPage = new LoginPage();
		homePage = new HomePage();
		masterPage = new MasterPage();
		thinkiamHomePage = loginPage.login(prop.getProperty("client"), prop.getProperty("username"), prop.getProperty("password"));
		homePage = thinkiamHomePage.goToThinkNumber();	
		TestUtil.SwitchToChildWindow();
		TestUtil.WaitTill_PageLoads(10);
		masterPage = homePage.goToMaster();
		homePage.clearAlertMessage();
		rLReportingLedgerListsPage = masterPage.goToRLReportingLedgerListsPage();
	}
	
	@DataProvider
	public Object[][] getTestData(){
		Object data[][] = TestUtil.getTestData(sheetName);
		return data;
	}
	
	@Test(priority=1, dataProvider="getTestData", enabled = true)
	public void addRLTest(String rlName, String dispName, String dashDispName, String type, String subType, String calc, String status) throws InterruptedException {
		rLReportingLedgerListsPage.addRL(rlName, dispName, dashDispName, type, subType, calc, status);
		Thread.sleep(1000);
		Assert.assertEquals(rLReportingLedgerListsPage.readAlertMessage(), "Reporting Ledger Added Successfully");
	}
	

	@Test(priority=2, dataProvider="getTestData", enabled = true)
	public void duplicateRLTest(String rlName, String dispName, String dashDispName, String type, String subType, String calc, String status) throws InterruptedException {
		rLReportingLedgerListsPage.addRL(rlName, dispName, dashDispName, type, subType, calc, status);
		Thread.sleep(1000);
		Assert.assertEquals(rLReportingLedgerListsPage.readAlertMessage(), "Reporting Ledger Name Already Available");
	}
	
	
	@AfterMethod
	public void tearDown() {
		//homePage.logoutThinkNumbers();
		TestUtil.SwitchToMainWindow();
		thinkiamHomePage.logOut();
		driver.quit();
	}

}
